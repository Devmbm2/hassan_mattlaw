...
if (typeof changeParentQS === 'undefined') {
  function changeParentQS(field) {
    ...
    if (!field) {
      return; // Add a check for null/undefined field
    }
    var form = field.form;
    if (!form || !form.id) {
      return; // Add a check for null/undefined form or form.id
    }
    var sqsId = form.id + "_" + field.id;
    var typeField = form.elements.parent_type;
    var new_module = typeField.value;
    // Update the SQS globals to reflect the new module choice
    if (typeof QSFieldsArray !== 'undefined' && typeof QSFieldsArray[sqsId] !== 'undefined') {
      QSFieldsArray[sqsId].sqs.modules = [new_module];
    }
    ...
    sqs_objects[sqsId].modules = [new_module];
