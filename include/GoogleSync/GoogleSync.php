...
class GoogleSync
{
    ...
    public function getUserMeetings()
    {
        ...
        // $query = "SELECT id FROM fp_events WHERE FIND_IN_SET('^".$this->workingUser->id."^', multiple_assigned_users) AND date_start <= now() + interval 3 month";
        $query = "SELECT id FROM fp_events WHERE date_start <= now() + interval 12 month";
    ...
    public function pullEvent($event_remote, $event_local = null)
    {
        ...
        if (!isset($event_local) || empty($event_local)) {
            ...
            $creatorEmail = $event_remote->getCreator()->getEmail();
    ...
    public function updateSuitecrmMeetingEvent($event_local, $event_remote)
    {
        ...
        if(!empty($event_local->multiple_assigned_users) || $event_local->multiple_assigned_users != NULL){
    ...
    public function sendEmailToCreator($event ,$creatorEmail){
        ...
        $site_url = $sugar_config['site_url'];
            $query=" SELECT * FROM fp_events
            WHERE gsync_id IS NOT NULL
            AND gsync_id != ''
            AND NOT EXISTS (
            SELECT * FROM cases_fp_events_1_c
            ...
            // $mailer->Subject ="New Calendar Entry in Honey";
            // $mailer->Body = "New Calendar Meeting Event Sync with Honey with Name" . " " . "' <b>" . $FP_events_calendar->name . "</b> '" . " Please assigned this event to the case!";
            // $mailer->Body.= "<br><a href = '{$site_url}/index.php?module=FP_events&action=DetailView&record={$FP_events_calendar->id}'>Click to View Event</a>";
            // $mailer->AddAddress($email);
            //  $mailer->Send();
            //  echo "sent email to Creator of Event"; 
                $to =$email;
            ...
            $headers .= 'From: CRM <devmbm2@gmail.com>' . "\r\n";        
