 ...
 */
require_once 'custom/include/calendar-work/vendor/autoload.php';
class GoogleSync
{
    ...
    public function getUserEvents()
    {
        ...
        while ($row = $this->db->fetchByAssoc($result)) {
            $results[] = $row['id'];
            $event = new FP_events();
            $event->retrieve($row['id'], true, false);
            $events[] = $event;
        }
        return $events;
    }
    ...
    public function pushPullSkip($event_1, $event_2 = null)
    {
        ...
        if (!isset($event_2) || empty($event_2)) {
            switch (get_class($event_1)) {
                ...
                case "FP_events":
                    if ($event_1->deleted == '0') {
                     // We only push if the event is not deleted
                        $return = "push";
                    } else {
                        $return = "skip";
                    }
    ...
    public function pullEvent2($event_remote, $event_local = null)
    {

        if (!isset($event_local) || empty($event_local)) {
            $event = $this->createSuitecrmFPEvent($event_remote);
        } else {
            $event = $this->updateSuitecrmMeetingEvent($event_local, $event_remote);
        }
    ...
    public function delEvent2($event)
    {
        $event->deleted = '1';
        $event->gsync_id = '';
        return $this->setLastSync($event);
    }
    ...
    public function createSuitecrmFPEvent($event_remote)
    {
        $GLOBALS['log']->debug(__FILE__ . ':' . __LINE__ . ' ' . __METHOD__ . ' - ' . 'Creating New SuiteCRM Event');
        $event = new FP_events;
        $event->id = create_guid();
        $event->new_with_id = true;
        $event_local = $this->updateSuitecrmMeetingEvent($event, $event_remote);
        return $event_local;
    }
    ...
    public function doSync($id)
    {
        ...
        if (!isset($events)) {
            $GLOBALS['log']->fatal(__FILE__ . ':' . __LINE__ . ' ' . __METHOD__ . ' - ' . 'Unable to get Users Events');
            return false;
        }
        ...
        // First, we look for SuiteCRM events that are not on Google
        foreach ($events as $event) {
            
            $gevent = null; // Sanity Check. At the begining of each loop, $gevent is NULL.
            $gevent = $this->getGoogleEventById($event->gsync_id);
            $dowhat = $this->pushPullSkip($event, $gevent);
            echo $dowhat;
            echo $event->name;
            switch ($dowhat) {
                case "push":
                    $GLOBALS['log']->info(__FILE__ . ':' . __LINE__ . ' ' . __METHOD__ . ' - ' . 'Pushing Event: ' . $event->name);
                    $this->pushEvent($event, $gevent);
                    break;
                case "pull":
                    $GLOBALS['log']->info(__FILE__ . ':' . __LINE__ . ' ' . __METHOD__ . ' - ' . 'Pulling Event: ' . $event->name);
                    $this->pullEvent2($gevent, $event);
                    break;
                case "skip":
                    $GLOBALS['log']->info(__FILE__ . ':' . __LINE__ . ' ' . __METHOD__ . ' - ' . 'Skipping Event: ' . $event->name);
                    break;
                case "push_delete":
                    $GLOBALS['log']->info(__FILE__ . ':' . __LINE__ . ' ' . __METHOD__ . ' - ' . 'Deleting Event: ' . $event->name);
                    $this->delEvent($gevent, $event->id);
                    break;
                case "pull_delete":
                    $GLOBALS['log']->info(__FILE__ . ':' . __LINE__ . ' ' . __METHOD__ . ' - ' . 'Deleting Event: ' . $event->name);
                    $this->delEvent2($event);
                    break;
                default:
                    $GLOBALS['log']->fatal(__FILE__ . ':' . __LINE__ . ' ' . __METHOD__ . ' - ' . 'pushPullSkip() returned unknown value: ' . $dowhat . ' for event: ' . $event->name);
            }
        }
        ...
        foreach ($googleEvents as $gevent) {
            if (!$event = $this->getEventByEventId($gevent->getId())) {
                $event = null;
            }

            $dowhat = $this->pushPullSkip($gevent, $event);
            echo $dowhat;
            switch ($dowhat) {
                case "push":
                    $GLOBALS['log']->info(__FILE__ . ':' . __LINE__ . ' ' . __METHOD__ . ' - ' . 'Pushing Event: ' . $gevent->getSummary());
                ...
                case "push_delete":
                    ...
                    break;
                case "pull_delete":
                    $GLOBALS['log']->info(__FILE__ . ':' . __LINE__ . ' ' . __METHOD__ . ' - ' . 'Deleting Meeting: ' . $event->name);
                    $this->delEvent2($event);
                    break;
                default:
                    $GLOBALS['log']->fatal(__FILE__ . ':' . __LINE__ . ' ' . __METHOD__ . ' - ' . 'pushPullSkip() returned unknown value: ' . $dowhat . ' for event: ' . $gevent->getSummary());
