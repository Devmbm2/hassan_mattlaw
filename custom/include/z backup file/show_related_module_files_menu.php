<?php
	...
	$parent_bean = BeanFactory::getBean('Accounts', $_REQUEST['record']);
...
$header = '<div><input type="button" id = "generate_report" value="Download Zip" onclick="related_module_files_zip_download(\''.$_REQUEST['record'].'\');" style="margin:5px 5px 0px 0;font-size:x-small;float:right;position:relative;"><input type="button" id = "unselect_all"  onclick="unselect_all_it(); return false;"   value="Deselect All" style="margin:5px 5px 0px 0;font-size:x-small;float:right;position:relative;">
<input type="button" id = "select_all" onclick="select_all_it(); return false;"    value="Select All" style="margin:5px 5px 0px 0; font-size:x-small;float:right;position:relative;">
...
if(!empty($related_modules_data)){
	foreach($related_modules_data AS $subpanel_name => $record){
		foreach($record AS $data){
			...
			if($subpanel_name == 'Soft Documents' || $subpanel_name == 'Hard Documents' ){
			...
			{
			$getfile_name = BeanFactory::getBean('Notes', $_REQUEST['id']);
			$notebean=$getfile_name;
			$file_name_db = $getfile_name->filename;
				$date2 = new \DateTime($getfile_name->date_entered);
				$file_date = $date2->format('m/d/Y');
			}
			else if($subpanel_name == 'Emails')
			{
		//	$getfile_name = BeanFactory::getBean('Emails', $_REQUEST['id']);
		      $getfile_name =BeanFactory::getBean('Notes')
					->retrieve_by_string_fields(
					array('Parent_type'=>'Emails' , 'Parent_id'=> $_REQUEST['id'] )
					);

			$email_bean=$getfile_name;
			$file_name_db = $getfile_name->document_name;
			echo $file_name_db; die; 
				$date2 = new \DateTime($getfile_name->date_entered);
				$file_date = $date2->format('m/d/Y');
			}
			else if($subpanel_name == 'Documents')
			{
			$getfile_name = BeanFactory::getBean('Documents', $_REQUEST['id']);
			$file_name_db = $getfile_name->document_name;
				$date2 = new \DateTime($getfile_name->date_entered);
		...
		else if($subpanel_name == 'Notes' && $notebean->filename=="") {
			$stream_html .= '<tr>';
			$stream_html .= '<td style="width: 50%;">' . $notebean->name . '</td>';
			$stream_html .= '<td style="width: 20%;">' . $subpanel_name . '</td>';
			$shortDescription = strlen($notebean->description) > 100 ? '<span class="description-short">' . substr($notebean->description, 0, 100) . '...</span>
			   <span class="description-full" style="display: none;">' . htmlentities($notebean->description) . '</span>
			...
			: $notebean->description;
			$stream_html .= '<td style="width: 20%;"><span class="description-short">' . $shortDescription . '</td>';
			$stream_html .= '<td style="width: 20%;">' . $file_date . '</td>';
			$stream_html .= '<td style="width: 20%;"></td>';
			$stream_html .= '<td style="width: 50%; text-align: center;"><input id="notes_id" name="checkbox" value="' . $notebean->id . '" type="checkbox" rel="' . $subpanel_name . '"/></td>';
			$stream_html .= '</tr>';
			
		}
		else if($subpanel_name == 'Emails' && $email_bean->has_attachment !==1) {
			$stream_html .='<tr>';
			$stream_html .='<td style="width:50%;">'. $email_bean->name .'</td>';
			$stream_html .='<td style="width:20%;">'. $subpanel_name .'</td>';
			$stream_html .='<td style="width:20%;"><a href="index.php?action=DetailView&module=Emails&record=' . $email_bean->id .' " target="_blank">View</a></td>';
			$stream_html .='<td style="width:20%;">'. $file_date .'</td>';
			$stream_html .='<td style="width:20%;"></td>';
			$stream_html .='<td style="width:50%;text-align:center;"><input id="emails_id"  name="checkbox" value="'. $email_bean->id .'" type="checkbox" rel="'. $subpanel_name .'"/></td>';
			$stream_html .='</tr>';
	...
	$stream_html .='
	<input type="button" id = "back_to" value="Go Back" onclick="ClosePopup_down(this); return false;" style="margin:15px 0 10px 0;">
	<input type="button" id = "select_all" onclick="select_all_it(); return false;"  " value="Select All" style="margin:15px 0 10px 0;">
	<input type="button" id = "unselect_all" onclick="unselect_all_it(); return false;"   value="Deselect All" style="margin:15px 0 10px 0;">
	<input type="button" id = "generate_report" value="Download Zip" onclick="related_module_files_zip_download(\''.$_REQUEST['record'].'\');" style="margin:15px 0 10px 0;">';
