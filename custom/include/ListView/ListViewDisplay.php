...
class ListViewDisplay {
	...
	protected function buildExportLink($loc = 'top')
    {
        ...
        // if($module == 'Cases' || $module == 'Contacts' || $module == 'Accounts' || $module == 'AOK_KnowledgeBase'){
        $result=$GLOBALS['db']->query('select * from config where name="export_doc_Modules"');
        $record=$GLOBALS['db']->fetchByAssoc($result);
        $allModules=str_replace("quot","",preg_replace('/[^A-Za-z0-9_\-]/', '', explode(',',$record['value'])));
        // print_r($allModules);die();
        $checkModuleExistinDBOrNot=false;
        foreach($allModules as $module2){

            if($module2==$module)
            {
                $checkModuleExistinDBOrNot=true;
                break;
            }
        }
        if($checkModuleExistinDBOrNot){
        	$sql = "SELECT * from config WHERE category = 'ExportDocPassword'";
        $result = $db->query($sql);
        $row = $db->fetchByAssoc($result);
        $current_password = $row['value'];
        $test = "<a href='javascript:void(0)' class=\"parent-dropdown-action-handler\" id=\"export_listview_". $loc ." \" onclick=\"test();\">{$app_strings['LBL_EXPORT']}</a>";
        echo "
        ...
        function test(){
            var password;
            password = prompt('Please enter your password to export!',' ');
            if (CryptoJS.MD5(password)=='".$current_password."'){
              alert('Access Granted!');
              return sListView.send_form(true, '{$this->seed->module_dir}', 'index.php?entryPoint=export','{$app_strings['LBL_LISTVIEW_NO_SELECTED']}');
            }
            else
            {
