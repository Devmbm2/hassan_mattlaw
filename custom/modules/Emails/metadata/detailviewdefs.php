    ...
    array (
      ...
      array (
        ...
        0 => 'EDIT',
          //1 => 'DUPLICATE',
        2 => 'DELETE',
        3 => createCustomButton('PLEA_Pleadings', 'Create Pleading'),
        4 => createCustomButton('NEG_Negotiations', 'Create Negotiation'),
        5 => createCustomButton('Documents', 'Create Document'),
        6 => createCustomButton('DISC_Discovery', 'Create Discovery'),
          //3 => 'FIND_DUPLICATES',
          /*4 =>
          array (
...
$viewdefs['Emails']['DetailView']['templateMeta']['includes'] =
    array (
        array (
  'file' => 'custom/modules/Emails/js/quickCreateModal.js',
        ),
    );

function createCustomButton($module, $label) {
        return array(
            'customCode' => '<input type=button onclick="openQuickCreateModal(\'' . $module . '\',\'&document_name={$bean->name}&attachments={$bean->id}&IsModelDropdown=true\',\'{$bean->from_addr_name}\');" value="' . $label . '">' .
