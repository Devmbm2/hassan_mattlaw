...
{if $contextMenus}
    <script type="text/javascript">
        ...
        $(document).ready(function () {
             ...
             $('#main_table').DataTable({
                 ...
                 initComplete: function () {
                     ...
                     this.api().columns([4]).every( function () {
                         ...
                         $.ajax({
                             url:"index.php?module=FP_events&action=getCases",
                             type: "post",
         ...
         function eventsNameSearch(){
             $('#search_events').on( 'keyup', function () {
                 let search_value2 = $(this).val();
                 if (search_value2.length >= 1) {
                     $.ajax({
                         url:"index.php?module=FP_events&action=index&return_module=FP_events&return_action=DetailView&clear_query=true&filter_module=" + search_value2,
                         type: "post",
                         data: {events_by_name:search_value2},
                         success:function(result){
         ...
         function eventsPurposeSearch(){
             $('#events_purpose').on('change', function(){
                 let purpose_value = $(this).find(":selected").val();
                 let case_value = $('#case_other').find(":selected").val();
                 let user_value0 = $('#events_users').find(":selected").val();
                 let user_value = '^'+user_value0+'^';
                 $.ajax({
                     url:"index.php?module=FP_events&action=index&return_module=FP_events&return_action=DetailView&clear_query=true&filter_module=fp_events_fields_search",
                     type: "post",
                     data: {
                     ...
                     success:function(result){
                         if($(result).find("#tbody_id").length == 1){
                            ...
                            $(".pageNumbers").html($(result).find(".pageNumbers")[0]);
                             //callFunctions();
                             //addDropdownValues(purpose_value,case_value,user_value0);
                         }else{
                             $("#tbody_id").empty();
                             $("#tbody_id").append(`<tr class="odd"><td valign="top" colspan="11" class="dataTables_empty">No data available in table</td></tr>`);
                             $('#pagination > td > table > tbody > tr > td:nth-child(3) > div').html('(0 - 0 of 0)');
         ...
         function eventsCaseSearch(){
             $('#case_other').on('change', function(){
                 let case_value = $(this).find(":selected").val();
                 let purpose_value = $('#events_purpose').find(":selected").val();
                 let user_value0 = $('#events_users').find(":selected").val();
                 let user_value = '^'+user_value0+'^';
                 $.ajax({
                     url:"index.php?module=FP_events&action=index&return_module=FP_events&return_action=DetailView&clear_query=true&filter_module=fp_events_fields_search",
         ...
         function eventsUserSearch(){
             $('#events_users').on('change', function(){
                 let user_value0 = $(this).find(":selected").val();
                 let user_value = '^'+user_value0+'^';
                 let case_value = $('#case_other').find(":selected").val();
                 let purpose_value = $('#events_purpose').find(":selected").val();
                 $.ajax({
...
// {*======Call Multiple Functions======*}
         function addDropdownValues(purpose_value,case_value,user_value0){
             setTimeout(function(){
                 $('#events_purpose').val(purpose_value);
                 $('#case_other').val(case_value);
                 $('#events_users').val(user_value0);
             }, 2000);
         }
