...
class CustomDocumentsController extends DocumentsController{
    ...
    public function action_getAllRelatedWorkflows(){
	   ...
	   $workflows="";
            ...
            <table class="table table-striped" id = "case_table" style="width:821px;"  >
            <thead>
            <tr>
         ...
         $query="Select * from aow_workflow Where flow_module='Documents'";
        ...
        //  pre($result);
         while($row = $db->fetchByAssoc($result)) {
            ...
            {
                    $get_id=$row['id'];
                    $sql="UPDATE `aow_workflow` SET `status`='Inactive' WHERE id='$get_id'";
                    $db->query($sql);
                    $sql2="SELECT * FROM aow_workflow WHERE id='$get_id'";
                    $result2=$db->query($sql2);
                    $workflow_related=$db->fetchByAssoc($result2);
                    // pre($workflow_related);
...
// pre($all_actions_related_to_workflow);
                    $workflows.='
                    <tr>
                        <td style="">
    ...
    function action_StatusActiveAndInactive(){
        global $db;
         ...
         foreach ($workflow_ids as $id) {
            $sql="UPDATE `aow_workflow` SET `status`='Active' where id='{$id}'";
            $db->query($sql);
        }
  	die;
    }
    function action_document_sent_out_status(){
        $bean=BeanFactory::getBean('Documents',$_REQUEST['record_id'][0]);
        $currentDateTime = date("Y-m-d H:i:s");
        $bean->sent_date=$currentDateTime;
        // pre();
        $bean->save();
