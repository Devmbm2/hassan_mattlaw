        ...
        initComplete: function () {
           this.api().columns().every( function () {
				...
				this.api().columns([9]).every( function () {
					var column = this;
					var select = $('<select name = "attorney" class="dt-search-select" id = "attorney" multiple><option value="">(No filter applied)</option></select>')
						.prependTo( $(column.header()))
						/*.on( 'change', function () {
							var val = $.fn.dataTable.util.escapeRegex(
								$(this).val()
