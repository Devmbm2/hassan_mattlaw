...
{if $hideTable == false}
	<div class="list-view-rounded-corners">
		...
		<input type="text" name="search" id="casesearch" class="form-control" placeholder="Search By Case Name">
	<thead>
...
let beforeAppend = $("#tbody_id").html();
	$('#casesearch').on( 'keyup', function () {
		let searcheditem = $('#casesearch').val();
		if (searcheditem !== "") {
			AjaxRequest('index.php?module=Cases&action=index&return_module=Cases&return_action=DetailView&clear_query=true',searcheditem);
	...
	function AjaxRequest(requestUrl,searcheditem){
		$.ajax({
			url: `${requestUrl}`,
			type: "post",
			data: {SearchData: searcheditem}, //add the FormData object to the data parameter
			success: function (response, status, jqxhr) {
				let searcheditem = $('#casesearch').val();
				if (searcheditem != "") {
				$("#tab_cases").html("");
				$("#tab_cases").append($(response).find("#tab_cases").html());
				ReinitializeCode();
				}
			},
...
var fuzzyNum =function(x){
 ...
 $.fn.dataTableExt.oSort['numbercase-desc']=function(x, y){
    ...
    function DataTableCreatedOnCondition(languageKey,columnNumber,id){
            ...
            this.api().columns().every( function () {
                    ...
                    this.api().columns([columnNumber]).every( function () {
                        ...
                        var select = $('<select class="dt-search-select" id = "'+id+'" ><option value="NoFilterApply">(No filter applied)</option></select>')
							...
							if(columnNumber==3){
								$(column.header()).append("<br>Status");
							}
							if(columnNumber==4){
								$(column.header()).append("<br>Type");
							}
							if(columnNumber==8){
								$(column.header()).append("<br>Assigned Lawyer");
								/*$.ajax({
        ...
        function SearchingOnBehalfOfChangeOfDropDown(requestUrl){
			var SENDDataToListViewFile={};
			if($("#status").val()==""){
				SENDDataToListViewFile.status="";
			}else if($("#status").val()!='NoFilterApply' && $("#status").val()!=''){
				SENDDataToListViewFile.status=$("#status").val();
			}
			if($("#type").val()==''){
				SENDDataToListViewFile.type="";
			}else if($("#type").val()!='NoFilterApply' && $("#type").val()!=''){
				SENDDataToListViewFile.type=$("#type").val();
			}
			if($("#assignedLawyer").val()!='NoFilterApply'){
				SENDDataToListViewFile.assigned_user_id=$("#assignedLawyer").val();
			}
			if($("#assistantLaywer").val()!='NoFilterApply'){
				SENDDataToListViewFile.assistentLawyerID=$("#assistantLaywer").val();
			}
			console.log(SENDDataToListViewFile);
				$.ajax({
					url: `${requestUrl}`,
					type: "post",
					data: SENDDataToListViewFile,
					success: function (response, status, jqxhr) {
						var setStatusDropdown=$("#status").val();
						var setTypeDropdown=$("#type").val();
						var setAssignedLawyerDropdown=$("#assignedLawyer").val();
						var setAssistentLawyerDropdown=$("#assistantLaywer").val();

						$("#tab_cases").html("");
						$("#tab_cases").html($(response).find("#tab_cases").html());
						ReinitializeCode();
						$("#status option[value='"+setStatusDropdown+"']").prop('selected', true);
						$("#type option[value='"+setTypeDropdown+"']").prop('selected', true);
						setTimeout(
						function()
						{
							$("#assignedLawyer option[value='"+setAssignedLawyerDropdown+"']").prop('selected', true);
						}, 300);
						setTimeout(
						function()
						{
							$("#assistantLaywer option[value='"+setAssistentLawyerDropdown+"']").prop('selected', true);
						}, 1000);
...
function ReinitializeCode(){
	DataTableCreatedOnCondition('case_status_dom',3,'status');
	DataTableCreatedOnCondition('complaint_type_list',4,'type');
	DataTableCreatedOnCondition('assigned_lawyer_cases_list',8,'assignedLawyer');
	$('#tab_cases').DataTable( {
		"ordering": false,
			columnDefs : [
					{
		...
		this.api().columns().every( function () {
				...
				var column = this;
				var select = $('<select name = "attorney" id="assistantLaywer" ><option value="NoFilterApply">(No filter applied)</option></select>')
					.prependTo( $(column.header()).empty())
					.on( 'change', function () {
					...
					if(column[0][0]==9){
						$(column.header()).append('<br>Assistant');
					}
					/*$.ajax({
						url: `index.php?module=Cases&action=GetAllUsers`,
						type: "get",
						...
						$.ajax({
                            url: 'index.php?module=Cases&action=get_case_assistant_options',
                            type: 'POST',
                            contentType: 'application/x-www-form-urlencoded',
                            dataType: 'text',
                            data: 'sugar_body_only=true',						
	...
	$("#status").on("change", function(){
		SearchingOnBehalfOfChangeOfDropDown("index.php?module=Cases&action=index&return_module=Cases&return_action=DetailView&clear_query=true");
	});
	$("#type").on("change", function(){
		SearchingOnBehalfOfChangeOfDropDown("index.php?module=Cases&action=index&return_module=Cases&return_action=DetailView&clear_query=true");
	});
	$("#assignedLawyer").on("change", function(){
		SearchingOnBehalfOfChangeOfDropDown("index.php?module=Cases&action=index&return_module=Cases&return_action=DetailView&clear_query=true");
	});
	$("#assistantLaywer").on("change", function(){
		SearchingOnBehalfOfChangeOfDropDown("index.php?module=Cases&action=index&return_module=Cases&return_action=DetailView&clear_query=true");
	});
}
ReinitializeCode();
