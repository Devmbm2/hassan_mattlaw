...
let beforeAppend = $("#tbody_id").html();
	$('#casesearch').on( 'keyup', function () {
		...
		if (searcheditem !== "") {
			AjaxRequest('index.php?module=Cases&action=SearchCases',searcheditem);
	...
	function AjaxRequest(requestUrl,searcheditem){
			$('#tab_cases').DataTable( {
					...
					columns:[
						...
						{data:'status'},
						{data:'type'},
                        {data:'DaysCurrentStatus'},
						{data:'date_of_incident_c'},
						{data:'TotalCaseLength'},
						{data:'UserName'},
						{data:'assistant_lawyer'},
						{data:'estimated_case_value'},
						{data:'insurance_summary'},
						{data:'trial_conference_hearing'},
						{data:'Info'},
					]
        ...
        initComplete: function () {
           this.api().columns().every( function () {
				...
				this.api().columns([3]).every( function () {
					var column = this;
					var select = $('<select class="dt-search-select" id = "status"><option value="">(No filter applied)</option></select>')
    ...
    function DataTableCreatedOnCondition(languageKey,columnNumber,id){
		...
		$('#tab_cases').DataTable( {
            ...
            this.api().columns().every( function () {
                    ...
                    this.api().columns([9]).every( function () {
					var column = this;
					var select = $('<select name = "attorney" id="assistantLaywer"><option value="">(No filter applied)</option></select>')
						.prependTo( $(column.header()))
						.on( 'change', function () {
							var val = $.fn.dataTable.util.escapeRegex(
								$(this).val()
        ...
        function SearchingOnBehalfOfChangeOfDropDown(requestUrl,DataObjectPassInAjaxRequest){
		$('#tab_cases').DataTable( {
                        ajax: {
                            url: `${requestUrl}`,
                            type: "post",
                            data: DataObjectPassInAjaxRequest,
                        },
                        ordering: false,
                        searching:false,
                        paging: false,
            ...
            $("#status").on("change", function(){
				let DataobjectPassTocontroller={status:this.value,type:$("#type").val(),assignedLawyer:$("#assignedLawyer").val(),AssistantLaywer:$("#assistantLaywer").val()};
                SearchingOnBehalfOfChangeOfDropDown("index.php?module=Cases&action=searchByStatus",DataobjectPassTocontroller);
            });
            $("#type").on("change", function(){
				let DataobjectPassTocontroller={status:$("#status").val(),type:this.value,assignedLawyer:$("#assignedLawyer").val(),AssistantLaywer:$("#assistantLaywer").val()};
                SearchingOnBehalfOfChangeOfDropDown("index.php?module=Cases&action=searchByType",DataobjectPassTocontroller);
            });
			$("#assignedLawyer").on("change", function(){
				let DataobjectPassTocontroller={status:$("#status").val(),type:$("#type").val(),assignedLawyer:this.value,AssistantLaywer:$("#assistantLaywer").val()};
                SearchingOnBehalfOfChangeOfDropDown("index.php?module=Cases&action=searchByAssignedLawyer",DataobjectPassTocontroller);
            });
			$("#assistantLaywer").on("change", function(){
				let DataobjectPassTocontroller={status:$("#status").val(),type:$("#type").val(),assignedLawyer:$("#assignedLawyer").val(),AssistantLaywer:this.value};
                SearchingOnBehalfOfChangeOfDropDown("index.php?module=Cases&action=searchByAssistentLawyer",DataobjectPassTocontroller);
            });
	//$('#status').select2();
	//$('#status').hide();
