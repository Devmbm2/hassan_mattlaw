...
$sql = "SELECT
	cc.contact_role as contact_role
FROM
	contacts_cases cc
WHERE
	cc.deleted = 0 AND cc.case_id = '{$case_id}'";
$result = $db->query($sql, true);
$related_contacts = $app_list_strings['relationship_to_case_list'];
$stream_html .='<tr><td><select style="height:50%;width: 60%;" name="list_of_case_related_contacts_role" id="list_of_case_related_contacts_role" >';
$stream_html .='</select></td></tr>';
	$stream_html .='<tr><td><input type="button" id = "send_doc" value="Send Document" onclick="send_doc(\''.$case_id.'\');"></td></tr></tbody>
</table>';
