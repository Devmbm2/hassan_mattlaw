...
$(document).ready(function() {
    ...
    function sendAjaxForLoadRecordsForReport(formData,forGraph=""){
    ...
    if (Input) {
      ...
      function GetResponseTable(response) {
        // console.log($(response.responseText).find('textarea[id="encoded_array"]')[0].value);
            ...
            formData=formData+"&sugar_body_only=true";
            http_fetch_async(
  ...
  function generateGraph(){
            let chartStatus = Chart.getChart("myChart"); // <canvas> id
              ...
              formData.push({ name: 'pdf_check', value: 'no' });
              var formData = $.param(formData);
              sendAjaxForLoadRecordsForReport(formData,'generateJsonForGraph');
              setTimeout(function() {
              var chartmonths=[];
              var jsonData = JSON.parse($('#encoded_array').val());
              var CountRecords=[];
              var periodRadio = document.getElementsByName("period_radio");
              if(periodRadio[2].checked && periodRadio[2].value === "day"){
                  var dateDropdown = document.getElementById("all_day_d");
                  for (var i = 0; i < dateDropdown.options.length; i++) {
                    var option = dateDropdown.options[i];
                    var dateText = option.text;
                    chartmonths.push(dateText);
                  }
                  $.each(jsonData, function(index, value) {
                    ...
                    const month_name = [
                      ...
                      chartmonths.push(month_name[index % 12]);
                    });
              }
