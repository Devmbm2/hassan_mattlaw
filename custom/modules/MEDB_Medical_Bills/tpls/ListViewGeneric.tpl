...
{if $hideTable == false}
	<div class="list-view-rounded-corners">
		<table id="main_table" cellpadding='0' cellspacing='0' border='0' class='list view table-responsive'>
			<input type="text" name="search_bill" id="search_bills" class="form-control" placeholder="Search Running Bills/Liens by Name">
	<thead>
...
$(document).ready(function () {
	...
	let pagination_value = $('.pageNumbers').text();
	let extracted_pagination_value =  pagination_value.slice(0,12);
	billsNameSearch();
	// {*======Search Data by Medical Bills Name======*}
	function billsNameSearch(){
		$('#search_bills').on( 'keyup', function () {
			let search_value2 = $(this).val();
			if (search_value2.length >= 1) {
				$.ajax({
					url:"index.php?module=MEDB_Medical_Bills&action=index&return_module=MEDB_Medical_Bills&return_action=DetailView&clear_query=true&filter_module=" + search_value2,
					type: "post",
					data: {bills_by_name:search_value2},
					success:function(result){
						if($(result).find("#tbody_id").length == 1){
							$("#main_table").remove();
							$(".list-view-rounded-corners").append($(result).find("#main_table"));
							dataTableAdjustment();
						}else{
							$("#tbody_id").empty();
							$("#tbody_id").append(`<tr class="odd"><td valign="top" colspan="11" class="dataTables_empty">
				...
				$('.pageNumbers').text(extracted_pagination_value);
			}
		});
	}

	//===== DataTable bugs fixing ======
