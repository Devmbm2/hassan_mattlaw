...
$(document).ready(function() {
	...
	// {*======Search Data By Note Type======*}
	function notesTypeSearch(){
		$('#note_type').on('change', function(){
			let search = $(this).find(":selected").val();
			$.ajax({
				url:"index.php?module=Notes&action=index&return_module=Notes&return_action=DetailView&clear_query=true&filter_module=notes_field_search",
				type: "post",
				data: { note_type_value: search },
				success:function(result){
					if($(result).find("#tbody_id").length == 1){
						...
						//notesTypeSearch();
						//styleDropdowns();
						//setTimeout(function(){ $('#note_type').val(search); }, 2000);
					}else{
						$("#tbody_id").empty();
	...
	// {*======Search Data by Notes Name======*}
	function notesNameSearch(){
		const main_table = $("#main_table").html();
		$('#search_note').on( 'keyup', function () {
			let search_value = $(this).val();
			if (search_value.length >= 1) {
				$.ajax({
					url:"index.php?module=Notes&action=index&return_module=Notes&return_action=DetailView&clear_query=true&filter_module=notes_name_search",
					type: "post",
					data: {notes_by_name:search_value},
					success:function(result){
						if($(result).find("#tbody_id").length == 1){
							$("#tbody_id").empty();
							$("#tbody_id").append($(result).find("#tbody_id > tr"));
