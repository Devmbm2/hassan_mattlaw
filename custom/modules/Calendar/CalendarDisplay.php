...
class CalendarDisplay {
	...
	public function display(){
		...
		// print_r("<pre>");print_r($cal->items);die;
		...
		// pass deadline task
		$deadline_task = array();
		$sql = "select tasks.*,tasks_cstm.* from tasks inner join tasks_cstm on tasks.id=tasks_cstm.id_c where tasks_cstm.type_of_todo_c = 'Deadline_Court' AND tasks.deleted = 0";
        $result = $GLOBALS['db']->query($sql, true);
        while ($row = $GLOBALS['db']->fetchByAssoc($result)) {
            $now = time();
            $your_date = strtotime($row['date_due']);
            $datediff = $now - $your_date;
            $date_check = $datediff;
            $findingNumberOfDays=round($date_check / (60 * 60 * 24));
            if($findingNumberOfDays==10 || $findingNumberOfDays==7 || $findingNumberOfDays==3){
            	$deadline_task[] = array("id" => $row['id'], "name" => $row['name'], "days" => $findingNumberOfDays);
            }
        }
        //print_r($deadline_task);
        // die();
        $ss->assign("deadline_tasks_list", $deadline_task);
        if(empty($deadline_task)){
        $count_zero = 0;
        ...
        $ss->assign("empty_deadline_list", $count_zero);
		//end deadline task
