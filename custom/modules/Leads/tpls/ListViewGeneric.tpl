...
{if $contextMenus}
    ...
    {literal}
        ...
        $(document).ready(function() {
            ...
            leadsLiveSearch();
            function leadsLiveSearch(){
                $('#search').on( 'keyup', function () {
                    let search_value2 = $(this).val();
                    if (search_value2.length >= 1) {
                        $.ajax({
                            url:"index.php?module=Leads&action=index&return_module=Leads&return_action=DetailView&clear_query=true&filter_module=" + search_value2,
                            type: "post",
                            data: {search_lead:search_value2},
