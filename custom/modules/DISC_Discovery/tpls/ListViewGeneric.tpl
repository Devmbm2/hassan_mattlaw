...
{if $hideTable == false}
	...
	<input type="text" name="disc_discovery_search" id="disc_discovery_search" class="form-control" placeholder="Search by Document Name">
...
function mainTable(){
	$('#main_table').DataTable({
		...
		initComplete: function () {
			...
			this.api().columns([3]).every( function () {
				...
				$.ajax({
					url:"index.php?module=DISC_Discovery&action=getCases",
					type: "post",
			...
			this.api().columns([8]).every( function () {
				var column = this;
				var select = $('<select name = "case_assistant" id = "case_assistant" class="dt-search-select"><option value="">(No filter applied)</option></select>')
			...
			this.api().columns([7]).every( function () {
				var column3 = this;
				var select3 = $('<select name = "case_assigned_to" id ="case_assigned_to" class="dt-search-select"><option value="no_filter">(No filter applied)</option></select>')
						.prependTo( $(column3.header()));
				$.ajax({
					url:"index.php?module=DISC_Discovery&action=getUsers",
					type: "post",
					success:function(result){
						let decode = JSON.parse(result);
						$.each(decode, function (key, value) {
	...
	// {*======Live Search Search field=======*}
            function DiscoveryLiveSearch(){
                $('input[name="disc_discovery_search"]').on( 'keyup', function () {
                    let search_value2 = $(this).val();
                    //console.log(search_value2.length);
                    if (search_value2.length >= 1) {
                        $.ajax({
                            url:"index.php?module=DISC_Discovery&action=index&return_module=DISC_Discovery&return_action=DetailView&clear_query=true",
                            type: "post",
                            data: {search_distDiscovery:search_value2},
                            success:function(result){
								console.log(result);
                                if($(result).find("#tbody_id").length == 1){
                                    ...
                                    $("#tbody_id").empty();
                                    $("#tbody_id").append(`<tr class="odd"><td valign="top" colspan="12" class="dataTables_empty">
                                    <pre class="tab">                                                                 No data available in table</pre></td></tr>`);
                    ...
                    if (search_value2.length == 0){

                                $("#tbody_id").empty();
                                $("#tbody_id").append(beforeAppend);
                                $(".pageNumbers").html($(main_table).find(".pageNumbers")[0]);
	...
	// {*========Live Search for Case Assigned To Field========*}
	function caseAssignedToSearch(){
		$('#case_assigned_to').on('change', function(){
			let search_value3 = $(this).find(":selected").val();
			$.ajax({
				url:"index.php?module=DISC_Discovery&action=index&return_module=DISC_Discovery&return_action=DetailView&clear_query=true&filter_module=" + search_value3,
				type: "post",
	...
	// {*========Search for Case Assistant Field=========*}
